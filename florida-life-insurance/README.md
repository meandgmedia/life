...
- Components: `Header`, `Nav` (desktop), `MobileNav` (slide-in panel), `Footer`,
  `Button` (primary/secondary/ghost), `Card` (generic — specialized cards like
  `ProductCard`/`BlogCard` are built in the phases that first need them)
...
- One route: `/` — a placeholder homepage that exists to demonstrate the
  design system working end-to-end (hero, cards, CTA section). **The full
  conversion-focused homepage from the brief is Phase 2** — this is
